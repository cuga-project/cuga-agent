...
async def main() -> int:
    if not TOKEN:
        print("SKIP: no TELEGRAM_BOT_TOKEN in .env"); return 0
    ...
    await eng.ensure_secret_connection(sa, PIECE, TOKEN); made.append(sa)
    ...
    await eng.ensure_secret_connection(ua, PIECE, TOKEN); made.append(ua)
    await eng.ensure_secret_connection(ub, PIECE, TOKEN); made.append(ub)
    ...
    per_user_ok = (ua != ub and await eng.connection_exists(ua)
                   and await eng.connection_exists(ub) and charlie_needs)
    ...
    print("\nPASS — shared cred reused by all users; per-user creds distinct; unconnected user "
          "→ connect prompt" if ok else "\nFAIL")
