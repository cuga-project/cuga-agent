...
async def new_replica():
    ...
    return runtime.ReactRuntime(model_factory=llm.default_model_factory,
                                agent_store=store, checkpointer=ckpt)
...
async def main() -> int:
    ...
    r1.upsert_agent(runtime.AgentSpec(name="geobot",
                    prompt="You are a geography assistant. Use your tools. Answer in one short line.",
                    mcp_servers=["cuga-geo"]), scope="acme/default/alice")
    a1 = await r1.run("geobot", "t1", "What is the capital of Japan? Just the city.",
    ...
    print("replica#2 sees agent from shared store:", bool(spec),
          "| mcp:", spec.mcp_servers if spec else None)
    ...
    a2 = await r2.run("geobot", "t1", "And its population? Just the number.",
                      scope="acme/default/alice")
    print("replica#2 follow-up:", a2)

    ok = (bool(spec) and other is None and "tokyo" in a1.lower()
          and any(x in a2.lower() for x in ("million", "13", "14", "37", "9", "populat")))
    print("\nPASS — agents + memory survive across replicas/restart, still isolated"
          if ok else "\nFAIL")
