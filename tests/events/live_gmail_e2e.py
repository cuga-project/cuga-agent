...
def _get(path):
    with urllib.request.urlopen(
        urllib.request.Request(SERVER + path, headers={"x-user-id": "admin"}), timeout=30
    ) as r:
        return json.load(r)
...
def main() -> int:
    ...
    check(
        "Gmail OAuth connection exists (per-user, in AP)",
        any(x.endswith("::gmail") for x in conns),
        next((x for x in conns if x.endswith("::gmail")), "—"),
    )
    ...
    check(
        "concierge armed the Gmail inbox watcher (not connect-needed)",
        armed and "connect needed" not in rep.lower(),
        rep[:80],
    )
    if gm:
        check(
            "Gmail watcher is a REAL AP flow (ap_flow_id recorded)",
            bool(gm.get("ap_flow_id")),
    ...
    if gwtok:
        ...
        env = {
            ...
            "event": {
                ...
                "payload": {
                    ...
                    "body": (
                        "Forwarding from finance — approve the Q3 vendor budget increase of "
        ...
        leak = any(
            m in ans
            for m in ("## New Variables Created", "Execution output:", "We have a loop", "delegate_to_")
        )
        check("synthetic fire (40KB forwarded email) answered ok", bool(body.get("ok")) and bool(ans))
        check(
            "answer is a SUMMARY, not leaked deliberation",
        ...
        if ok:
            try:
                ...
                record(
                    ...
                    "(deliberation-leak + size regression gate)",
                    source="live_gmail_e2e.py",
                )
