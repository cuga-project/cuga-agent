...
ROSTER = os.path.join(os.path.dirname(__file__), "..", "..", "supervisor_agents.yaml")

# (label, fire/chat text, acceptable picks)  — some events legitimately fit >1 specialist
CASES = [
    ("fire/github-new_pr", "[github/new_pr] PR opened on acme/api: 'Fix pagination off-by-one', "
     "+84 -12, 3 files. Review it and summarize the risk.", {"pr_reviewer"}),
    ("fire/github-new_issue", "[github/new_issue] Issue filed on acme/api: 'Login 500s on "
     "submit'. Triage severity and suggest a label.", {"incident_triage"}),
    ("fire/github-new_star", "[github/new_star] acme/api got a new star from user dana. "
     "Post a thank-you note.", {"repo_watcher"}),
    ("fire/github-new_release", "[github/new_release] Release v2.1 published on acme/api. "
     "Summarize the changelog for the team.", {"repo_watcher", "webpage_summarizer"}),
    ("fire/gmail-new_email", "[gmail/new_email] New email — from cfo@acme.com, subject 'Invoice "
     "overdue', body: 'Vendor invoice #221 is 30 days overdue.' Summarize and suggest an action.",
     {"mailbot"}),
    ("fire/gmail-new_attachment", "[gmail/new_attachment] Email with attachment resume.pdf — "
     "subject 'Application: Senior SWE'. Judge the candidate.", {"resume_judge", "mailbot"}),
    ("fire/box-new_file", "[box/new_file] A new file landed in the Box folder: "
     "'dana_resume.pdf'. Judge the resume.", {"resume_judge"}),
    ("fire/slack-new_reaction", "[slack/new_reaction] A :bug: reaction was added to a message in "
     "#incidents: 'checkout-api 500s spiking'. Triage it as an incident.", {"incident_triage"}),
    ("fire/slack-new_slack_user", "[slack/new_slack_user] Dana Lee (Backend engineer) joined the "
     "workspace. Send an onboarding brief.", {"support_digest", "onboarding_buddy", "mailbot"}),
    ...
    ("chat/papers", "any new arXiv papers on mixture-of-experts this week?",
     {"papers", "research_compass"}),
    ("ambig/email-about-pr", "[gmail/new_email] New email — subject 'Please review PR #99', "
     "body: 'Can someone look at the pagination PR today?' Summarize this email.", {"mailbot"}),
    ("ambig/issue-vs-incident", "[github/new_issue] Issue on acme/api: 'Production outage — "
     "all requests failing'. Triage it.", {"incident_triage"}),
]
...
async def main() -> int:
    ...
    sup = CugaSupervisor(agents=cfg.agents,
    ...
    for label, text, want in CASES:
        ...
        bad += (not ok)
        selfies += (not PICKS)
        print(f"  {'✓' if ok else '✗'} {label:26} picked={picked:20} "
