...
def main() -> int:
    ...
    c, r = _req("POST", "/api/concierge",
                {"text": "when a resume lands in Box, judge fit vs the JD and email me",
                 "thread_id": "web:stage2"}, headers={"X-User-Id": "admin"}, timeout=300)
    reply = json.dumps(r.get("reply", r))
    checks.append(("Box resume watcher armed (push)", "push" in reply.lower() or "arm" in reply.lower()
                   or "box" in reply.lower()))
    ...
    env = {"source": {"type": "channel", "name": "telegram", "thread_id": "gw:telegram:55501"},
           "event": {"kind": "message", "payload": {}}, "text": f"/start {lk.get('token')}",
           "agent": "concierge", "deliver": False}
    ...
    checks.append(("simulated telegram inbound → resolved+routed → answer",
                   c == 200 and (r.get("ok") is True)))
    ...
    print("NOTE: a REAL inbound message (you messaging the bot) completes the round-trip; AP then "
