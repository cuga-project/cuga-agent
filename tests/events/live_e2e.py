...
class Report:
    ...
    def ok(self, phase, name, cond, detail="", fail_detail=""):
        ...
        return self.add(phase, name, PASS if cond else FAIL,
                        detail if cond else (fail_detail or detail))
...
def _invoke_channel(name: str, thread: str, user: str, deliver: bool):
    ...
    payload = {"text": PROBE, "agent": "concierge", "deliver": deliver,
...
def ch_slack(r: Report):
    ...
    if not secret:
        print("     \033[33mwarning\033[0m: SLACK_SIGNING_SECRET unset — the /api/events/slack/events "
    ...
    if not chan:
        ...
        if not lst.get("ok"):
            return r.skip("slack", "slack round trip",
    ...
    _, posted = http("POST", "https://slack.com/api/chat.postMessage",
                     {"channel": chan, "text": f"[e2e] {PROBE}"}, sh, timeout=20)
    ...
    if secret:
        bad, _ = http("POST", BASE + "/api/events/slack/events", raw_body=raw, timeout=15,
                      headers={"Content-Type": "application/json",
                               "X-Slack-Request-Timestamp": stamp, "X-Slack-Signature": "v0=deadbeef"})
        r.ok("slack", "server rejects a badly-signed event (401)", bad == 401, fail_detail=f"HTTP {bad}")
    else:
        r.skip("slack", "server rejects a badly-signed event",
...
def ch_discord(r: Report):
    ...
    if not chan:
        ...
        if not isinstance(guilds, list) or not guilds:
            return r.skip("discord", "discord round trip",
        ...
        if not text:
            return r.skip("discord", "discord round trip", "no text channel visible — set DISCORD_TEST_CHANNEL_ID")
        chan = text[0]["id"]
    ...
    time.sleep(3)
    _, msgs = http("GET", f"https://discord.com/api/v10/channels/{chan}/messages?limit=10",
                   headers=dh, timeout=20)
...
def ch_telegram(r: Report):
    ...
    if not r.ok("telegram", "bot token valid (getMe)", who.get("ok"),
...
def flow_alive(sub) -> tuple[bool, str]:
    ...
    if not (body or {}).get("ap_flow"):
        return False, (f"DANGLING: subscription points at AP flow {fid}, which does not exist in "
...
def _arm_and_verify(r: Report, phase: str, utter: str, mode: str, thread: str,
    ...
    if not r.ok(phase, f"{phase}: concierge accepted the utterance", code == 200,
                fail_detail=f"HTTP {code}"):
        return
    sub, is_new = _match_sub(before, mode)
    if not r.ok(phase, f"{phase}: a {mode} subscription exists after arming", bool(sub),
    ...
    if not ap_live:
        ...
        return
    if not r.ok(phase, f"{phase}: AP flow id recorded on the subscription", bool(sub.get("ap_flow_id")),
                sub.get("ap_flow_id", ""), fail_detail="armed locally but no ap_flow_id"):
        return
...
def flow_cron(r: Report, ap_live: bool, created: list):
    ...
    _arm_and_verify(r, "CRON", "every day at 9am send me new arxiv papers on mixture of experts",
                    "CRON", f"web:{RUN}:cron", ap_live, created)
...
def flow_poll(r: Report, ap_live: bool, created: list):
    ...
    _arm_and_verify(r, "POLL", "watch bitcoin every 2 minutes and ping me on any move",
                    "POLL", f"web:{RUN}:poll", ap_live, created)
...
def flow_push(r: Report, facts: dict, created: list):
    ...
    for app, utter in cases:
        ...
        before = {s.get("id") for s in _subs()}
        code, rep = srv("POST", "/api/concierge", {"text": utter, "thread_id": f"web:{RUN}:push:{app}"},
                        timeout=300)
        reply = str(rep.get("reply", ""))
        low = reply.lower()
        print(f"     → {reply[:140]}")
