...
BENCH = [
    ...
    _c(
        "review new PRs on acme/api as they arrive and post a verdict",
        source="github",
        event="new_pr",
        config=R,
        strict=False,
    ...
    _c(
        "as soon as an issue lands on acme/api, triage it by severity",
        source="github",
        event="new_issue",
        config=R,
        strict=False,
    ...
    _c(
        "when my repo acme/api gets a new star, post a thank-you", source="github", event="new_star", config=R
    ),
    _c(
        "when code is pushed to main on acme/api, audit the diff", source="github", event="new_push", config=R
    ),
    _c(
        "when a new discussion opens on acme/api, summarize it",
        source="github",
        event="new_discussion",
        config=R,
    ),
    _c(
        "when a comment is posted on a discussion in acme/api, flag blockers",
        source="github",
        event="new_discussion_comment",
        config=R,
    ),
    _c(
        "when a new branch is created on acme/api, check the naming convention",
        source="github",
        event="new_branch",
        config=R,
    ),
    _c(
        "when a collaborator is added to acme/api, log their access",
        source="github",
        event="new_collaborator",
        config=R,
    ),
    _c(
        "when a repo label is created on acme/api, announce it",
        source="github",
        event="new_repo_label",
        config=R,
    ),
    _c(
        "when a milestone is created on acme/api, draft a plan",
        source="github",
        event="new_milestone",
        config=R,
    ),
    _c(
        "when a new release is published on acme/api, summarize the changelog",
        source="github",
        event="new_release",
        config=R,
    ),
    ...
    _c(
        "when I'm requested to review a PR on acme/api, summarize the diff",
        source="github",
        event="new_review_request",
        config=R,
    ),
    _c(
        "when someone mentions me on github in acme/api, summarize the context",
        source="github",
    ...
    # ── slack (8) ────────────────────────────────────────────────────────────────────────────────
    _c(
        "when a message is posted in #incidents, triage it",
        source="slack",
        event="new_channel_message",
        config={"channel": "incidents"},
    ),
    _c(
        "when a message gets a :bug: reaction in slack, triage it",
        source="slack",
        event="new_reaction",
        config={"emoji": "bug"},
    ),
    _c(
        "when a reaction is removed from a task message, re-open it", source="slack", event="reaction_removed"
    ),
    _c(
    ...
    _c(
        "when a new user joins the workspace, send an onboarding brief",
        source="slack",
    ...
    _c(
        "when someone sends the bot a link on telegram, summarize it",
        source="telegram",
        event="new_channel_message",
        strict=False,
    ),
    ...
    _c(
        "when there's a new pin on my pinterest board, share it",
        source="pinterest",
        event="new_pin",
        strict=False,
    ),
    _c(
        "when a new board is created on pinterest, announce it",
        source="pinterest",
        event="new_board",
        strict=False,
    ),
    ...
    _c(
        "when my youtube channel posts a new video, share it",
        source="youtube",
        event="new_video",
        strict=False,
    ),
    _c(
        "when a new item appears in the rss feed https://example.com/feed.xml, summarize it",
        source="rss",
        event="new_item",
        strict=False,
    ),
    _c(
        "when someone reacts in discord with :tada:, thank them",
        source="discord",
        event="new_reaction",
        strict=False,
    ),
...
def test_bench_zero_wrong_at_high():
    ...
    for utter, want, spec in _score():
        ...
        if want["kind"] != "push" or not want["source"]:
            bad.append((utter, f"resolved high but the label says kind={want['kind']!r}", spec))
        elif (spec.source, spec.event) != (want["source"], want["event"]):
            bad.append(
                (
                    utter,
                    f"high but wrong trigger: got {spec.source}/{spec.event}, "
                    f"want {want['source']}/{want['event']}",
                    spec,
                )
            )
