...
def test_registry_github_is_fully_synth_fireable():
    ...
    for t in gh:
        ...
        roots = {
            v.split("{{trigger.")[1].split(".")[0].split("}}")[0]
...
def test_every_ap_row_builds_its_own_flow():
    ...
    for t in triggers.rows():
        if t.backend != "ap":
            continue
        f = flows.build_push_flow(agent="a", thread_id="t", prompt="p", source=t.app, event_kind=t.event)
...
def test_direct_rows_render_placeholder_not_ap_piece():
    f = flows.build_push_flow(agent="a", thread_id="t", prompt="p", source="box", event_kind="new_folder")
...
EVAL = [
    ...
    ("when a comment is posted on a discussion in o/r, flag blockers", ("github", "new_discussion_comment")),
    ...
    ("when I'm requested to review a PR on o/r, summarize the diff", ("github", "new_review_request")),
    ("when someone mentions me on github in o/r, summarize the context", ("github", "new_gh_mention")),
    # gmail (4)
    ...
    ("when the team is @mentioned in a slack channel, draft an answer", ("slack", "new_slack_mention")),
    ...
    ("when someone posts in #help on discord, answer from the docs", ("discord", "new_channel_message")),
    ...
    ("when a calendar event is updated, tell me what changed", ("google_calendar", "new_or_updated_event")),
...
def test_classifier_eval_set_routes_every_trigger():
    ...
    misrouted = [(t, want, classify.source_of(t)) for t, want in EVAL if classify.source_of(t) != want]
...
def test_classifier_eval_covers_every_ap_and_channel_trigger():
    ...
    expected = {
        t.key for t in triggers.rows() if t.app not in ("webhook", "telegram")
    }  # webhook needs no arming; telegram
...
def test_classifier_poll_vs_cron_new_items():
    ...
    assert (
        classify.classify("every 5 minutes check for new MoE papers and tell me only if there are new ones")
        == "POLL"
    )
...
def test_classifier_delivery_clause_is_not_a_gmail_source():
    ...
    assert classify.source_of("when a resume lands in my Box folder, email me the fit") == ("box", "new_file")
...
def test_direct_match_applies_config_filters():
    ...
    st.upsert(_watcher("w3", event="new_channel_message", config={"pattern": "https?://"}, app="telegram"))
    # emoji filter
    ...
    assert [
        s.id for s in direct_events.match(st, "slack", "new_reaction", emoji="bookmark", channel="C9")
    ] == ["w2"]
    # pattern filter (a URL-bearing telegram message)
    assert [
        s.id for s in direct_events.match(st, "telegram", "new_channel_message", text="see https://x.com")
    ] == ["w3"]
