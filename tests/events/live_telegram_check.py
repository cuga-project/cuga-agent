...
def main():
    ...
    if not tok:
        print("SKIP — no TELEGRAM_BOT_TOKEN")
        return 0
    ...
    if want_handle:
        check(
            "bot username matches EVENTS_TELEGRAM_BOT_USERNAME",
    ...
    if chat:
        _, r = _http(
            "POST", f"{api}/sendMessage", {"chat_id": chat, "text": "✅ live_telegram_check: delivery leg OK"}
        )
    ...
    check(
        "descriptor: reply→chat, send_text_message wiring",
