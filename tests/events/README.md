...
Two tiers: **offline** (no server, no creds — the safety net) and **live** (real server + real APIs).

## Offline — run this first
```bash
.venv/bin/python -m pytest tests/events -q          # fast, no network
```
Covers the whole layer across `test_events_*.py` — envelope · classify · flow builders ·
subscriptions · concierge dry-run · connectors · credentials · isolation · per-mode flows ·
native scheduler · poll-state delta · slack/discord/telegram direct · delivery · the Studio API
contract · and the doc-contract tests (`test_events_api_contract.py`) that fail the build if a route
or trigger is added without documenting it in `api/` / `setup/`.
...
`make sync` is **not** needed — the live harnesses are pure stdlib. Use the phase targets above
(`test-suite-now` / `test-suite-flows`) rather than one giant run, so a budget overrun can't silently
skip the tail.

`live_suite.py` is the **behavioural** harness: every agent answers a NOW question (asserted against
`meta.mcp`, so an agent can't pass by answering from the model's memory instead of calling its tools),
then every flow mode arms. It reports **four** outcomes, not two — `PASS` · `FAIL` · `XFAIL` (a logged
gap, reason printed) · `XPASS` (a gap just closed → go delete the expectation). Only `FAIL` reddens the
bar.
...
(Gateway socket / Telegram long-poll) is simulated by driving the exact `/invoke` envelope it posts,
...
| **Telegram** (direct) | `live_telegram_check.py` | token (getMe) + delivery leg + arm the direct long-poll backend |
