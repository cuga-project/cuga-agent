...
non-``Answer`` events (such as ``SlashSkillInvoked``, ``ThreadIdChanged``,
``SlashSuggestions``) short-circuited to bare ``self.data`` and skipped the
``event: <name>\\n`` prefix and the SSE terminator blank line, causing the
client to see raw JSON glued to the next event.
"""
...
def test_default_format_wraps_thread_id_changed_event():
    """Regression: ``ThreadIdChanged`` must drive the frontend's thread rotation.

    The event lives or dies by its ``event:`` prefix — without it the
    EventSource handler never fires, the frontend never rotates its local
...
def test_default_format_wraps_slash_suggestions_event():
    """Regression: unknown-command suggestions need the wrapper to render chips."""
    payload = json.dumps(
        {
            "raw_input": "/sumarize",
            "suggestions": [
