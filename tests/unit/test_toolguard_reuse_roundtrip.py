...
async def test_frontend_export_import_roundtrip_preserves_tool_guards_and_guards_enabled(
    ...
    exported = _policy_to_frontend_dict(FINANCE_POLICY_MODEL_DUMP)
    ...
    _assert_finance_guards_intact(storage.policies[0], guards_enabled=True)
...
async def test_sdk_export_import_roundtrip_preserves_tool_guards_and_guards_enabled(
    ...
    try:
        ...
        # SDK export uses model_dump — guards_enabled must be present
        assert exported_data[0]["guards_enabled"] is True
        ...
        _assert_finance_guards_intact(dest.policies[0], guards_enabled=True)
    finally:
        os.unlink(tmp_path)
