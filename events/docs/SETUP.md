...
- **Browser access (split topology):** `EVENTS_CORS_ORIGINS` — the origins allowed to call the
  eventing service from a browser. **Unset it derives the CUGA origin from `CUGA_URL`**, which is what
  makes `make up-noap` work; the CE deploy sets it explicitly to the deployed UI's route. Get this
  wrong and the Studio's concierge box prints *nothing at all* — the browser cancels the request
  before it is sent, so there is no error to see.
- **Running a local stack while a deployment is live:** `LOCAL_HONOR_SLACK` / `LOCAL_HONOR_DISCORD` /
  `LOCAL_HONOR_TELEGRAM` `=false`. Bot credentials are single-consumer — Telegram's `getUpdates`
  allows one poller per token and two Discord Gateway sessions both answer — so without these a
  laptop and a deployment fight over every message. Unset means honour, so a deployment that sets
  none of them is unaffected.
- **Arming:** `EVENTS_FLOW_REUSE=1` restores the old reuse-an-existing-flow behaviour. Off by
  default, because it answered "Nothing new created" *after* a human had confirmed an arming.
...
[rosters/README.md](../examples/rosters/README.md) for the full table; then `make reload`.

### Adding a sub-agent (builder guide)

A sub-agent is **a skill, not a deployment**: a name, a prompt, and tools. Append a block to your
roster YAML (`events/examples/rosters/default.yaml` by default):
...
## Redirect / callback URLs — what to update, and when
