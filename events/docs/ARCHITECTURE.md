...
This document is the architectural record. For step-by-step instructions see [SETUP.md](SETUP.md);
for what to *type* see
...
| | |
|---|---|
| Turn it on | `./events/deploy/3_state_store.sh` then `EVENTS_STATE_STORE=cuga-events-state ./2_deploy.sh` |
...
`src/cuga/backend/events/poll_state.py` · `events/deploy/2_deploy.sh`
