...
> **The live URLs are deliberately not in this repo.** It is open source, and the deployment
> is a real agent with real channel credentials — a published route is an open invitation.
> Ask the team for the two routes, or read them from your own deploy:
> `make ce-url` (or `events/deploy/.ce_urls.env`, which is gitignored).
...
> Building or deploying it yourself instead? → **[SETUP.md](SETUP.md)** (local) · **[../deploy/README.md](../deploy/README.md)** (Code Engine)
...
| | |
|---|---|
| **Studio (web UI)** | https://<your-cuga-core-route>/studio |
...
```bash
curl -s https://<your-cuga-events-route>/health
```
