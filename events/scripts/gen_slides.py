#!/usr/bin/env python
"""Generate events/docs/slides.html — the event-driven-agents deck — from the code itself.
...
Sources of truth (nothing in the deck is hand-typed twice):
  ...
  * src/cuga/backend/events/catalog.py  — the example utterances shown per trigger
    ...
    python events/scripts/gen_slides.py            # rewrite in place
    python events/scripts/gen_slides.py --check    # exit 1 if out of date (used by the consistency test)
...
OUT = ROOT / "events/docs" / "slides.html"
...
<p>PUSH is where the trigger registry lives — everything on the next slides is a PUSH trigger.</p>""",
    ...
    # 4 — architecture
    svg = ROOT / "events/docs" / "architecture" / "system.svg"
...
<ul class="tight">
  ...
  <li><b>Direct backend</b> — CUGA already receives Slack Events / the Discord Gateway, so a watcher
    ...
    try:
        ...
        n_roster = len(
            (_yaml.safe_load(open(ROOT / "events" / "examples" / "rosters" / "default.yaml")) or {}).get(
                "agents"
            )
            or []
        )
...
channels / triggers / webhooks ──► /invoke {{agent: "cuga", source, event}}
                              ...
                              reads its roster: events/examples/rosters/default.yaml
...
<p><b>Benchmarked in CI:</b> {b['cases']} hand-labelled cases (utterance → expected FlowSpec) —
   ...
   bugs on its first run. Full walkthrough: <code>events/docs/nl_to_flow.html</code>.</p>""",
        )
    )
...
<p class="dim"><b>Critical path to safe:</b> inject <code>X-User-Id</code> from auth · durable
   ...
   <code>events/docs/DEPLOYMENT_ISSUES.md</code>. Naming the gate is how we ship v1 honestly.</p>""",
        )
    )
...
<ul class="tight">
  ...
  <li><b><code>make test-fire</code></b> — the only harness that waits for a genuine schedule tick:
    ...
    # roadmap (mirrors events/docs/ROADMAP.md — that file is the source of truth)
    slides.append(
        _slide(
...
<p class="dim">Discipline: a capability is "done" only when it passes both benchmark levels.
   Full plan: events/docs/roadmap.html · gaps: events/docs/todos_actions/GAPS.md</p>""",
        )
    )
...
<!-- GENERATED by events/scripts/gen_slides.py — do not hand-edit. Sources: triggers.py + catalog.py -->
<html lang="en"><head>
...
def main() -> int:
    ...
    if current == fresh:
        ...
        return 0
    if check:
        print("✗ slides.html is stale — run: python events/scripts/gen_slides.py", file=sys.stderr)
        return 1
    OUT.write_text(fresh)
    print(
        f"✓ wrote events/docs/slides.html — {fresh.count('class="slide')} slides "
