#!/usr/bin/env python
"""Regenerate the `const EX = [...]` block in events/docs/api/examples.html from the ONE source of
...
data array is spliced in, so the doc board can never drift from the Studio Examples tab again.
    ...
    python events/scripts/gen_examples.py            # rewrite in place
    python events/scripts/gen_examples.py --check    # exit 1 if out of date (used by the consistency test)
...
HTML = ROOT / "events/docs" / "api" / "examples.html"
...
def main() -> int:
    ...
    if m.group(0).strip() == fresh.strip():
        ...
        return 0
    if check:
        print("✗ examples.html is stale — run: python events/scripts/gen_examples.py", file=sys.stderr)
        return 1
