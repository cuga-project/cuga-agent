...
> | **Event-driven agents** (channels · triggers · standing flows) | **`cuga start demo --events`** — Slack/Discord/Telegram chat, webhooks, cron/poll/push flows armed from natural language, one supervisor agent over a YAML roster (`EVENTS_SUPERVISOR=1`). Full stack incl. Activepieces + tunnels: `make up`. [Docs](events_docs/README.md) · [Setup](events_docs/SETUP.md) |
