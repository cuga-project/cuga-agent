...
GROUPS = [
    ("core", "Core — the reasoning seam", "Two endpoints do all the thinking. Everything else is "
     "plumbing around them."),
    ("flows", "Flows — the standing subscriptions", "A flow is one armed subscription. CUGA stores "
...
def E(verb, path, group, summary, *, tier, callers, auth="none", path_params=(), query=(),
      body=(), responses=(), notes="", try_it=True):
    return dict(verb=verb, path=path, group=group, summary=summary, tier=tier,
                callers=list(callers), auth=auth, path_params=list(path_params),
                query=list(query), body=list(body), responses=list(responses),
                notes=notes, try_it=try_it)


# The envelope every inbound surface builds. Repeated here because it IS the contract.
ENV_NOW = {"agent": "pricebot", "text": "what is the current price of bitcoin?",
           "deliver": False,
...
ENDPOINTS = [
    ...
    E("POST", "/invoke", "core",
      ...
      tier="core", callers=["ap", "cuga", "tests", "operator"], auth="gateway",
      body=[
          ("Direct agent call (NOW) — no channel, no concierge", ENV_NOW,
          ...
          ("A chat message routed through the concierge", {
              "agent": "concierge", "text": "every day at 9am send me the price of bitcoin",
              "deliver": False,
              "source": {"type": "channel", "name": "slack",
                         "thread_id": "gw:slack:C0BEYJ9NATB#1783630758.450769", "user": "U123"},
          ...
          ("A PUSH flow firing (Gmail → agent → Slack)", {
              "agent": "mailbot", "text": "summarize this email", "deliver": True,
              "source": {"type": "integration", "name": "gmail",
                         "thread_id": "default/default/local::gw:slack:C0BEYJ9NATB"},
              "event": {"kind": "new_email",
                        "payload": {"subject": "Q3 numbers", "from": "boss@corp.com",
                                    "body": "See attached."}}},
          ...
          ("A CRON flow firing on schedule", {
              "agent": "papers", "text": "new arxiv papers on mixture of experts",
              "deliver": False, "scope": "default/default/local",
          ...
          ("A POLL flow reporting a change", {
              "agent": "feed_watcher", "text": "tell me only about new items",
              "deliver": True,
              "source": {"type": "time", "name": "schedule",
          ...
          ("A Box file landing (direct poller)", {
              "agent": "resume_judge", "deliver": True, "scope": "default/default/local",
              ...
              "event": {"kind": "new_file",
                        "payload": {"file_id": "9", "name": "resume.pdf"}}},
          ...
          ("A generic webhook triage", {
              "agent": "incident_triage", "deliver": True,
              "text": "An external system POSTed to webhook 'monitoring'. Triage this payload:\n\n"
                      "{\n  \"alert\": \"HighCPU\",\n  \"service\": \"checkout-api\"\n}",
              "source": {"type": "channel", "name": "slack", "thread_id": "gw:slack:C0BEYJ9NATB"},
              "event": {"kind": "message",
                        "payload": {"alert": "HighCPU", "service": "checkout-api"}}},
          ...
          ("Account-linking handshake", {
              "agent": "concierge", "text": "/start 4f3c1a…",
          ...
          ("Minimal — the smallest thing that works", {
              "agent": "pricebot", "text": "bitcoin price?",
              "source": {"type": "time", "name": "cron"}, "event": {"kind": "runonce"}},
      ...
      responses=[
          (200, {"ok": True, "agent": "pricebot", "answer": "Bitcoin is currently **$63,240 USD**.",
                 "trace_id": "6f2a…",
                 "meta": {"agent": "pricebot", "backend": "cuga", "mcp": ["cuga-finance"],
                          "tools": ["get_crypto_price"], "ms": 6412}},
           "The answer, plus who produced it. <code>meta.mcp</code> is the honest signal."),
          (400, {"ok": False, "error": "invalid envelope: source.type 'carrier-pigeon' not in …"},
          ...
          (501, {"ok": False, "error": "concierge not configured"},
    ...
    E("POST", "/api/concierge", "core",
      ...
      body=[
          ...
          ("A one-shot question", {"text": "what is bitcoin worth?", "thread_id": "web:local"},
      ...
      responses=[
          (200, {"ok": True, "reply": "Cron flow set up — papers, daily at 09:00, delivering here.",
    ...
    E("GET", "/api/events/subscriptions", "flows",
      ...
      responses=[(200, {"scope": "default/default/local", "subscriptions": [
          {"id": "s1", "mode": "CRON", "target_agent": "papers", "deliver_to": ["telegram"],
           "ap_flow_id": "lqfzMLeItA", "flow_name": "cron-papers", "status": "active",
    ...
    E("POST", "/api/events/subscriptions/{sub_id}/pause", "flows",
      "Pause a flow — disables it in Activepieces, not just in CUGA's row.",
      tier="ui", callers=["studio", "operator"], path_params=[("sub_id", "subscription id", "s1")],
      responses=[(200, {"ok": True, "id": "s1", "status": "paused"}, ""),
    ...
    E("POST", "/api/events/subscriptions/{sub_id}/resume", "flows",
      "Re-enable a paused flow in Activepieces.",
      tier="ui", callers=["studio", "operator"], path_params=[("sub_id", "subscription id", "s1")],
      responses=[(200, {"ok": True, "id": "s1", "status": "active"}, ""),
                 (404, {"ok": False, "error": "subscription not found"}, "")]),

    E("DELETE", "/api/events/subscriptions/{sub_id}", "flows",
      "Delete a flow — removes it from Activepieces too.",
      tier="ui", callers=["studio", "operator", "tests"], path_params=[("sub_id", "subscription id", "s1")],
      responses=[(200, {"ok": True, "id": "s1", "deleted": True}, ""),
                 (404, {"ok": False, "error": "subscription not found"}, "")],
    ...
    E("GET", "/api/events/subscriptions/{sub_id}/flow", "flows",
      ...
      responses=[
          (200, {"ok": True, "subscription": {"id": "s1", "ap_flow_id": "lqfzMLeItA"},
                 "ap_flow": {"id": "lqfzMLeItA", "status": "ENABLED",
    ...
    E("GET", "/api/events/runs/{run_id}", "runs",
      ...
      responses=[
          (200, {"ok": True, "run": {"id": "r1", "status": "SUCCEEDED",
          ...
          (200, {"ok": True, "run": {"id": "r1", "status": "FAILED"}, "answer": None,
                 "trigger_payload": None, "error": "401 Bad credentials"},
    ...
    # ── studio reads ──────────────────────────────────────────────────────────
    E("GET", "/api/events/status", "studio",
      "What the events layer can do right now. The UI decides what to show from this.",
      tier="ui", callers=["studio", "tests"],
      responses=[(200, {"ok": True, "enabled": True, "scope": "default/default/local",
                        "concierge_backend": "react", "worker_backend": "cuga",
                        "backends": ["react", "cuga"], "ap_configured": True,
                        "project_grain": "tenant",
                        "features": {"now": True, "cron": True, "poll": True, "push": True,
                                     "channels_inbound": True}},
                  "Every feature except <code>now</code> is gated on Activepieces being configured.")]),

    E("GET", "/api/events/channels", "studio",
      "Inbound chat channels and their connect state.", tier="ui", callers=["studio", "tests"],
      responses=[(200, {"channels": [{"name": "web", "status": "connected"},
                                     {"name": "slack", "status": "connected"},
                                     {"name": "discord", "status": "connected"},
                                     {"name": "telegram", "status": "not_connected"}]}, "")]),

    E("GET", "/api/events/integrations", "studio",
      "Box / GitHub / Gmail connection state, and which backend owns each.",
      tier="ui", callers=["studio", "tests"],
      responses=[(200, {"integrations": [
          {"name": "box", "status": "connected", "backend": "direct",
           "note": "DIRECT backend — CUGA polls Box with BOX_DEV_TOKEN (no AP, no OAuth)."},
          {"name": "github", "status": "auto_connect_pending", "backend": "ap",
    ...
    E("GET", "/api/events/examples", "studio",
      "The tagged catalog of example utterances.", tier="ui", callers=["studio", "tests"],
    ...
    E("GET", "/api/events/setup-guides", "studio",
      ...
      tier="ui", callers=["studio"],
      responses=[(200, {"public_url": "https://your-domain.ngrok-free.app", "guides": [
          {"app": "gmail", "kind": "integration", "connect": "oauth",
           "creds": [{"key": "EVENTS_OAUTH_GMAIL_CLIENT_ID", "present": True, "scope": "tenant"}],
           "steps": ["Add https://…/api/events/connect/gmail/callback as a redirect URI"],
           "conn_status": "not_connected", "connected": False,
           "connection_scope": "user", "needs_connection": True}]},
        "")],
    ...
    E("GET", "/api/events/agents", "agents",
      ...
      responses=[(200, {"scope": "default/default", "agents": [
          {"name": "pricebot", "prompt": "You answer questions about asset prices…",
           "backend": "cuga", "mcp_servers": ["cuga-finance"], "channels": ["web", "slack"],
           "integrations": [], "access": [], "restricted": False,
           "examples": ["what is the current price of bitcoin?"], "can_use": True}]},
    ...
    E("POST", "/api/events/agents", "agents",
      "Create or upsert a worker agent. Idempotent by name.", tier="ui", callers=["studio"], auth="builder",
      body=[("A new agent", {
          "name": "webpage_summarizer", "backend": "cuga",
         ...
         ("An agent needing a per-user integration", {
             "name": "resume_judge", "backend": "cuga", "prompt": "Judge the resume against the JD.",
             "mcp_servers": ["cuga-text"], "channels": ["web"],
         ...
         ("Restricted to a role", {
             "name": "payroll_bot", "backend": "cuga", "prompt": "Answer payroll questions.",
             "mcp_servers": ["cuga-text"], "channels": ["web"], "access": ["finance", "admin"]},
         ...
         ("The minimum viable agent", {"name": "echo", "prompt": "Repeat the user's message back."},
         ...
         ("A ReAct-backend agent", {
             "name": "quickbot", "backend": "react", "prompt": "Answer briefly.",
             "mcp_servers": ["cuga-web"], "channels": ["web"]},
    ...
    E("PUT", "/api/events/agents/{name}", "agents",
      ...
      body=[("Edit the prompt", {"name": "webpage_summarizer", "backend": "cuga",
            ...
            ("Give an existing agent a tool it was missing", {
                "name": "mailbot", "backend": "cuga",
                "prompt": "Summarise the email in the event payload.",
                "mcp_servers": ["cuga-text", "cuga-web"], "channels": ["web", "slack"],
    ...
    E("POST", "/api/events/slack/events", "inbound",
      ...
      body=[("URL verification handshake",
            ...
            ("A human message", {"type": "event_callback", "event": {
                "type": "message", "text": "what is bitcoin worth?", "channel": "C0BEYJ9NATB",
                "user": "U123", "ts": "1783630758.450769"}},
            ...
            ("A threaded reply", {"type": "event_callback", "event": {
                "type": "message", "text": "and ethereum?", "channel": "C0BEYJ9NATB",
                "user": "U123", "ts": "1783630801.11", "thread_ts": "1783630758.450769"}},
             ...
             "<code>#&lt;ts&gt;</code> suffix."),
            ("The bot's own message (ignored)", {"type": "event_callback", "event": {
                "type": "message", "text": "Bitcoin is $63,240.", "channel": "C0BEYJ9NATB",
                "bot_id": "B123", "ts": "1783630760.0"}},
             "<code>should_process</code> filters out bot messages, edits and joins. Without that the "
             "bot answers itself, forever.")],
      responses=[(200, {"ok": True}, "The ack. The answer arrives in Slack, not in this response."),
    ...
    E("POST", "/api/events/box/poll", "inbound",
      ...
      body=[("Server-tracked watermark (a standing poll)", {"folder_id": "0"},
            ...
            ("Manual poll from an explicit timestamp", {
                "folder_id": "0", "since": "2026-07-01T00:00:00-07:00",
                "agent": "resume_judge", "deliver_to": "slack", "deliver_target": "C0BEYJ9NATB"},
            ...
            ("Replay everything in the folder", {"folder_id": "0", "since": "1970-01-01T00:00:00Z"},
            ...
            ("Poll on someone else's behalf", {"folder_id": "0", "scope": "acme/prod/alice"},
      ...
      responses=[(200, {"ok": True, "folder": "0",
                        "processed": [{"id": "9", "name": "resume.pdf"}],
                        "newest": "2026-07-06T11:00:00-07:00", "trace_id": "a1b2…"},
                  "<code>newest</code> is the watermark to store as the next <code>since</code>."),
                 (401, {"ok": False, "error": "bad or missing X-Gateway-Token"}, ""),
                 (502, {"ok": False, "error": "401 Unauthorized"},
    ...
    E("POST", "/api/events/hook/{name}", "inbound",
      ...
      body=[("A monitoring alert", {"alert": "HighCPU", "service": "checkout-api",
                                    "value": 97, "threshold": 85, "severity": "page"},
            ...
            ("A CI failure (GitHub Actions)", {
                "action": "completed", "workflow_run": {
                    "name": "test", "conclusion": "failure", "head_branch": "feat/cuga-loops",
            ...
            ("A payment provider event", {
                "type": "charge.failed", "data": {"object": {
                    "id": "ch_3P…", "amount": 4900, "currency": "usd",
    ...
    E("GET", "/api/events/connect/{app}", "connect",
      ...
      tier="ui", callers=["browser", "studio"], try_it=False,
      path_params=[("app", "box · gmail · github · slack · telegram · discord", "gmail")],
      query=[("ownership", "<code>per-user</code> (default) or <code>tenant</code>", "per-user"),
    ...
    E("GET", "/api/events/connect/{app}/callback", "connect",
      "OAuth redirect target: exchange the code, create the AP connection.",
      tier="edge", callers=["browser"], try_it=False,
      path_params=[("app", "the app being connected", "gmail")],
      query=[("code", "authorization code from the provider", ""),
             ("state", "opaque; carries scope + ownership + return", "")],
      responses=[(200, "&lt;h3&gt;✅ gmail connected&lt;/h3&gt;", "HTML, for a human in a browser."),
    ...
    E("POST", "/api/events/connect/{app}/token", "connect",
      ...
      body=[("A GitHub PAT", {"token": "ghp_xxxxxxxxxxxxxxxx"},
            ...
            ("Shared across the tenant", {"token": "ghp_xxx", "ownership": "tenant"},
      ...
      responses=[(200, {"ok": True, "app": "github", "connection": "ea::default::local::github"}, ""),
                 (400, {"ok": False, "error": "missing 'token'"}, ""),
                 (400, {"ok": False, "app": "gmail",
    ...
    E("GET", "/api/events/connections", "connect",
      ...
      query=[("scope", "override the principal", "")],
      responses=[(200, {"scope": "default/default/local",
    ...
    # ── identity ──────────────────────────────────────────────────────────────
    E("GET", "/api/events/me", "identity",
      "The caller's profile: who they are, roles, linked channels, connections.",
      tier="ui", callers=["studio"], query=[("scope", "override the principal", "")],
      responses=[(200, {"scope": "default/default/local", "user_id": "local",
                        "email": "a@corp.com", "roles": ["admin"],
    ...
    E("POST", "/api/events/link/{channel}", "identity",
      "Issue a link token binding a chat account to this profile.",
      tier="ui", callers=["studio"], path_params=[("channel", "telegram · slack · discord", "telegram")],
      body=[("Issue a token", {}, "The body may be empty; the principal comes from headers."),
            ("Issue on behalf of another user", {"scope": "acme/prod/alice"},
             "The token binds whichever profile the scope names — so an admin can hand Alice a code "
             "to paste into her own Telegram.")],
      responses=[(200, {"ok": True, "channel": "telegram", "token": "4f3c1a…",
    ...
    # ── admin ─────────────────────────────────────────────────────────────────
    E("GET", "/api/events/admin/users", "admin", "List the tenant's users.",
      tier="ui", callers=["studio"], auth="admin",
      responses=[(200, {"users": [{"user_id": "local", "email": "a@corp.com", "roles": ["admin"]}]}, ""),
    ...
    E("POST", "/api/events/admin/users", "admin", "Add a user.", tier="ui", callers=["studio"], auth="admin",
      body=[("A regular user", {"user_id": "bob", "email": "b@corp.com", "roles": ["user"]},
             "<code>roles</code> defaults to <code>[\"user\"]</code> when omitted."),
            ("A builder", {"user_id": "carol", "email": "c@corp.com", "roles": ["builder"]},
             "Builders may create and edit agents (<code>POST/PUT /api/events/agents</code>)."),
            ("Another admin", {"user_id": "dave", "email": "d@corp.com", "roles": ["admin"]},
             "Admins pass both the admin and the builder gate."),
            ("With a password", {"user_id": "erin", "email": "e@corp.com", "roles": ["user"],
      ...
      responses=[(200, {"ok": True, "user": {"user_id": "bob", "email": "b@corp.com",
                                             "roles": ["user"]}}, ""),
                 (400, {"ok": False, "error": "missing user_id"}, ""),
                 (403, {"ok": False, "error": "admin only"}, ""),
                 (501, {"ok": False, "error": "user store not configured"}, "")]),

    E("POST", "/api/events/admin/channels/{channel}/arm", "admin",
      "Arm a channel's inbound flow.", tier="ops", callers=["operator", "studio"], auth="admin",
      path_params=[("channel", "slack · discord · telegram", "telegram")],
      body=[("Telegram (via AP)", {}, "No trigger input needed — the bot polls all its chats."),
            ("Discord (via AP, one channel)", {"channel": "1522408587958423675"},
            ...
            ("Arm for a specific tenant", {"scope": "acme/prod/alice"},
             "<code>scope</code> is stripped from the trigger input and used to pick the AP project "
             "and the connection.")],
      responses=[(200, {"ok": True, "channel": "telegram", "ap_flow_id": "URoIxzSflT"},
                  "The AP path."),
                 (200, {"ok": True, "channel": "slack", "backend": "direct",
                 ...
                 (200, {"ok": True, "channel": "discord", "backend": "direct",
                 ...
                 (403, {"ok": False, "error": "admin only"}, ""),
                 (501, {"ok": False, "error": "AP not configured"}, "")],
      notes="<code>make channels</code> calls this for every channel with a token in "
            "<code>.env</code>."),

    E("GET", "/api/events/admin/oauth-apps", "admin",
      "Which OAuth providers have a client id/secret configured. No secrets returned.",
      tier="ui", callers=["studio"], auth="admin",
      responses=[(200, {"apps": [{"app": "gmail", "configured": True},
                                 {"app": "box", "configured": False}]}, ""),
                 (403, {"ok": False, "error": "admin only"}, "")]),

    E("POST", "/api/events/admin/oauth-apps", "admin",
      "Enter a provider's OAuth client id/secret once, from the UI instead of .env.",
      tier="ui", callers=["studio"], auth="admin",
      body=[("Configure Gmail", {"app": "gmail", "client_id": "…apps.googleusercontent.com",
            ...
            ("Configure Box", {"app": "box", "client_id": "abc123",
                               "client_secret": "s3cret", "scopes": "root_readwrite"}, ""),
      ...
      responses=[(200, {"ok": True, "app": "gmail", "configured": True}, ""),
...
AUTH_CHIP = {
    "none": ("", ""),
    "gateway": ("gateway token", "Requires <code>X-Gateway-Token</code> when "
                                 "<code>GATEWAY_TOKEN</code> is set."),
    "admin": ("admin", "Admin role required. Open when no user store is configured."),
    "builder": ("builder", "Builder or admin role required."),
    "slack": ("slack signature", "Verified against <code>SLACK_SIGNING_SECRET</code>; "
                                 "<b>open when unset</b>."),
...
def render_endpoint(ep) -> str:
    ...
    chips = "".join(f'<span class="chip who" title="{e(ACTORS[c][1])}">{e(ACTORS[c][0])}</span>'
                    for c in ep["callers"])
    auth = f'<span class="chip auth" title="{a_help}">🔒 {a_label}</span>' if a_label else ""
    tlabel, _, thelp = TIERS[ep["tier"]]
    tier = (f'<span class="tier t-{ep["tier"]}" '
    ...
    if ep["try_it"]:
        ...
        ta = (f'<textarea class="tbody" rows="8" spellcheck="false">{e(first)}</textarea>'
              if ep["body"] else "")
        tryit = f"""
    <details class="try">
...
base.value=localStorage.getItem(BK)||'http://localhost:8100';
...
function applyFilters(){
  ...
  document.querySelectorAll('nav a').forEach(a=>{
    const ep=document.querySelector(a.getAttribute('href').replace('#','#'));
...
def render() -> str:
    ...
    for gid, gtitle, gblurb in GROUPS:
        ...
        nav += [f'<a href="#{slug(x)}"><span class="v v-{x["verb"].lower()}">{x["verb"]}</span>'
                f'{e(x["path"].replace("/api/events", ""))}</a>' for x in eps]
        body.append(f'<section id="g-{gid}"><h2>{gtitle}</h2><p class="sub">{gblurb}</p>'
                    + "".join(render_endpoint(x) for x in eps) + "</section>")

    actors = "".join(f'<tr><td><span class="chip who">{e(n)}</span></td><td>{e(d)}</td></tr>'
                     for n, d in ACTORS.values())
    n = len(ENDPOINTS)

    counts = {t: sum(1 for x in ENDPOINTS if x["tier"] == t) for t in TIERS}
    tfilter = ('<div class="tfilter"><button data-t="all" class="on">ALL</button>'
               + "".join(f'<button data-t="{t}">{lab} {counts[t]}</button>'
                         for t, (lab, _, _) in TIERS.items()) + "</div>")
    glance = "".join(
        f'<tr><td style="white-space:nowrap"><span class="tier t-{t}">{lab}</span>'
        f'<b>{counts[t]}</b></td><td>{helptext}<div class="note">'
        + " · ".join(f'<code>{e(x["verb"])} {e(x["path"])}</code>'
                     for x in ENDPOINTS if x["tier"] == t)
        + "</div></td></tr>"
...
def main() -> int:
    ...
    ap.add_argument("--check", action="store_true",
                    help="exit 1 if the spec is stale or a route is undocumented")
    a = ap.parse_args()
    ...
    if a.check:
        ...
        if current != fresh:
            print(f"{OUT.relative_to(REPO)} is STALE — run: python scripts/gen_api_spec.py",
                  file=sys.stderr)
            return 1
    ...
    print(f"✓ wrote {OUT.relative_to(REPO)} — {len(ENDPOINTS)} endpoints, "
