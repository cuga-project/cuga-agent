...
| **GitHub PR trigger needs the repo named** | The concierge can't guess `owner/repo`; the user must say it (*"when a PR opens on psf/requests…"*). | **Now auto-extracted** from the utterance (`owner/repo` regex) and wired into the trigger's required `repository` prop; the connection is normalized to the base `github` app (was wrongly `github_pr`). If the repo is missing, the concierge asks for it. |
| **GitHub PR/issue trigger creates a repo WEBHOOK on publish** | Publishing calls GitHub's create-hook API. A **fine-grained PAT** with repo *admin role* is NOT enough — it also needs the explicit **Webhooks: Read and write** permission, else GitHub returns 403 (AP surfaces it as `TRIGGER_UPDATE_STATUS` / "bad credentials"). | Reconnect with a fine-grained PAT granting **Webhooks: R/W** (+ Pull requests / Contents: Read) on the repo, or a **classic PAT** with `admin:repo_hook` + `repo`. The concierge now returns this as an actionable message instead of a raw AP error. |
