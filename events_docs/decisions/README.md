...
is Context → Decision → Consequences. Verified live (0001–0007: 2026-07-02; 0008–0009: 2026-07-15/17)
unless noted.
...
| [0005](0005-runtime-router-over-prebuilt-agents.md) | **Concierge = runtime router, not a factory** *(partially superseded by [0009](0009-single-agent-supervisor.md))* | The concierge SELECTS among builder-defined agents (answer-now / reuse-or-create-flow / decline); it never invents agents or tools. `provision_agent` removed. *(0009: routing moved INSIDE the one `cuga` supervisor; the concierge now routes to no agent, only compiles NL→Flow.)* |
