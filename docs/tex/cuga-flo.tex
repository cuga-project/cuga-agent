...
\coordinate (fa@tr) at (5.5,5.55);
\begin{scope}[on background layer]
  \node[frame, fit=(fa@bl)(fa@tr), inner sep=2pt] (fabox) {};
\end{scope}
\node[font=\footnotesize\bfseries\color{teal!60!black}] at (2.2,5.28)
      {FlowAgent (reasoning meta-agent)};

% DecisionAgent outer box with nested CugaAgent
\node[agent, minimum height=1.7cm] (da) at (0.6, 4.6) {};
\node[font=\small, text centered] at (0.6, 5.02) {\texttt{Decision}\\[-2pt]\texttt{Agent}};
\node[draw=blue!50, fill=blue!6, rounded corners=2pt,
      minimum width=2.1cm, minimum height=0.4cm, font=\tiny, text centered]
      at (0.6, 4.18) {\texttt{CugaAgent}};

% TaskAgents outer box with nested CugaAgent
\node[task, minimum height=1.7cm] (ta) at (0.6, 2.7) {};
\node[font=\small, text centered] at (0.6, 3.12) {Task\\[-2pt]Agents};
\node[draw=blue!50, fill=blue!6, rounded corners=2pt,
      minimum width=2.1cm, minimum height=0.4cm, font=\tiny, text centered]
