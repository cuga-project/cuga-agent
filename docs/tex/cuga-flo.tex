...
\coordinate (fa@bl) at (-0.4,2.05);
\coordinate (fa@tr) at (4.8,5.45);
\begin{scope}[on background layer]
  \node[frame, fit=(fa@bl)(fa@tr), inner sep=2pt] (fabox) {};
\end{scope}
\node[font=\footnotesize\bfseries\color{teal!60!black}] at (2.2,5.15)
      {FlowAgent (reasoning meta-agent)};

\node[agent] (da) at (0.95, 4.1) {\texttt{Decision}\\[-2pt]\texttt{Agent}};
\node[agent] (hm) at (3.5, 4.1) {\texttt{Hook}\\[-2pt]\texttt{reasoning}};
\node[task] (ta) at (2.2, 2.95) {Task\\Agents};
\node[font=\scriptsize\color{black!55}] at (2.2, 2.38) {CugaAgent$\times n$};

% ── FRAME (below FlowAgent) ────────────────────────────────
\node[policy] (pt) at (0.7, 0.95) {Task\\Policies};
\node[policy] (pd) at (2.2, 0.95) {Decision\\Policies};
\node[policy] (ph) at (3.7, 0.95) {Hook\\Policies};
\begin{scope}[on background layer]
...
\node[draw=orange!60, fill=orange!10, rounded corners=4pt,
      ...
      text centered, font=\scriptsize] (bridge) at (6.3,3.75)
...
% ── Execution side: compilation + engine (right) ───────────
\node[draw=black!45, fill=yellow!12, rounded corners=4pt,
      minimum width=2.6cm, minimum height=0.85cm, text centered, text width=2.4cm]
      (bpmn) at (10.4,6.6) {BPMN 2.0 Diagram};
\node[draw=black!45, fill=green!10, rounded corners=4pt,
      minimum width=2.6cm, minimum height=0.85cm, text centered, text width=2.4cm]
      (parser) at (10.4,5.35) {\texttt{BPMNParser}};
\node[draw=blue!50, fill=blue!8, rounded corners=4pt,
      minimum width=2.6cm, minimum height=1.0cm, text centered, text width=2.4cm]
      (engine) at (10.4,3.7) {\texttt{WorkflowEngine}\\\scriptsize(LangGraph demo)};
\node[draw=black!45, fill=gray!8, rounded corners=4pt,
      minimum width=2.6cm, minimum height=0.9cm, text centered, text width=2.4cm]
      (fs) at (10.4,2.3) {\texttt{FlowState}\\\scriptsize shared state};

\draw[flow, double] (bpmn) -- (parser);
\draw[flow, double] (parser) -- (engine)
      node[midway, xshift=-1.1cm, font=\scriptsize]{compile};
\draw[flow] (engine) -- (fs);
...
with its DecisionAgent, hook reasoning, and TaskAgents, all governed by the FRAME
$\mathcal{F}$---is decoupled from the execution side (right) by the
\texttt{MCPFlowBridge}. The bridge exposes the FlowAgent's reasoning tools
(\texttt{execute\_task}, \texttt{route\_gateway}, \texttt{evaluate\_hook},
\texttt{get\_static\_config}) and the engine's \texttt{run\_process} tool. A
replaceable \texttt{WorkflowEngine} (the included demo is built on LangGraph) parses
the BPMN, drives execution over \texttt{FlowState}, and calls back into the FlowAgent
at each control point. The FlowAgent presents a unified \texttt{invoke()} interface
to a parent \texttt{CugaSupervisor}.}
...
\subsection{Hook-Based Runtime Flow Adaptation}
