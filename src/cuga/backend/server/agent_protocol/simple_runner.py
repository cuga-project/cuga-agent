...
class SimpleAgentRunner:
    ...
    - Supports human-in-the-loop interrupts via ``auto_approve`` or by
      surfacing an ``input_required`` terminal event to the caller.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            auto_approve: When True, HITL interrupts are auto-confirmed and the
                ...
                to the caller as an ``input_required`` terminal event.
            caller_user_id: The ``user_id`` passed to CUGA's ``event_stream``
                and embedded in ``ActionResponse`` objects. Defaults to
                ``"agent_protocol_user"``; A2A subclass passes ``"a2a_user"``.
        """
    ...
    async def run(
        ...
        Args:
            ...
            approval: Optional structured approval signal extracted from the
                inbound message (e.g. ``{"action_id": ..., "confirmed": bool}``).
                Only consulted when the thread is parked on a HITL interrupt.

        Yields:
            AgentStreamEvent instances representing the agent's response.
        """
        ...
        try:
            ...
            if parked is not None:
                ...
                logger.info(
                    "Approval reply on parked thread %s: action=%s decision=%s",
                    thread_id,
            ...
            for _ in range(_MAX_AUTO_RESUMES + 1):
                ...
                logger.info(f"Auto-approving HITL action {pending.get('action_id')}")
            ...
            logger.warning(f"Event stream completed without a final answer (thread_id={thread_id})")
            yield AgentStreamEvent("final_answer", {"text": "Agent completed processing"}, final=True)

        except Exception as exc:
            logger.exception("Agent runner failed")
            yield AgentStreamEvent("error", {"text": f"Agent error: {type(exc).__name__}"}, final=True)
    ...
    def _decode(frame: Any) -> Tuple[Optional[str], Optional[str]]:
        ...
        if stripped.startswith("event:") or "\nevent:" in text:
            try:
                ...
                return ev.name, SimpleAgentRunner._payload_text(ev.data)
            except Exception:
                return None, None
        ...
        if stripped.startswith("data:"):
            ...
            if isinstance(obj, dict) and "name" in obj:
                return obj.get("name", "unknown"), SimpleAgentRunner._payload_text(obj.get("data"))
    ...
    def _payload_text(data: Any) -> str:
        ...
        if isinstance(data, dict):
            return SimpleAgentRunner._from_mapping(data)
        if isinstance(data, str):
            ...
            if s[:1] in ("{", "["):
                ...
                if isinstance(parsed, dict):
                    return SimpleAgentRunner._from_mapping(parsed)
            return data
        return str(data)
    ...
    def _input_required_event(self, pending: dict) -> AgentStreamEvent:
        """Terminal event that surfaces a pending HITL action to the caller.
    ...
    def _build_response(self, pending: dict, decision: bool, message: Optional[str]) -> ActionResponse:
