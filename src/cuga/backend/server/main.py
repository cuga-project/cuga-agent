    ...
    async def initialize_knowledge_engine(app_state, kb_config: "KnowledgeConfig") -> None:
        ...
        async def _warm():
            try:
                ...
                app_state.set_subsystem_status("knowledge", "ready", "Knowledge subsystem ready", warmup_result)
                logger.info("Knowledge subsystem warmup complete")
            except Exception as e:
                logger.exception("Knowledge subsystem warmup failed: %s", e)
                app_state.set_subsystem_status("knowledge", "failed", "Knowledge subsystem failed during warmup", {"error": str(e)})
