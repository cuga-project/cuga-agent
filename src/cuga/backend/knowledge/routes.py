...
def _result_to_chunk(r, include_score: bool) -> dict[str, Any]:
    """Serialize a SearchResult into the wire shape.

    Single helper so every code path emits identical chunk JSON — drift
    between the ``scope="all"`` branch and the single-scope branch was a
    real risk before this consolidation.
    ...
    if include_score:
        chunk["score"] = r.score
    # Hybrid-retrieval observability: surface ranks + fused score on
    # any chunk RRF actually fused. ``None`` means "leg didn't appear in
    # the input" (lexical leg empty for pre-FTS collections; dense leg
    # would only be None for lexical-only chunks).
...
def _group_by_source(results, scopes_searched: list[str]) -> dict[str, list]:
    """Group SearchResults by their ``scope`` tag, preserving rank order
    within each bucket. Pre-seeded with empty lists for every scope that
    was actually searched so the LLM can tell "searched but no hits" from
    "scope wasn't searched at all".

    Orphans (empty or unknown ``scope``) are **dropped**, not bucketed
    into a fallback. Bucketing an unknown chunk under the wrong scope
    would let the LLM cite it confidently as agent content when it might
    be session (or something else) — strictly worse than not seeing the
    chunk. If orphans become common we should fix the upstream code path
    that fails to tag them, not paper over it here.
    """
    by: dict[str, list] = {s: [] for s in scopes_searched}
    dropped = 0
    for r in results:
        if r.scope in by:
            by[r.scope].append(r)
    ...
    if dropped:
        logger.warning(
            "_group_by_source: dropped %d untagged result(s); "
            "scopes_searched=%s — fix the upstream code path that "
            "produced SearchResult without a valid ``scope``.",
            dropped,
            scopes_searched,
        )
    return by
