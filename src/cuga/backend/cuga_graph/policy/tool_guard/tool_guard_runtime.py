...
class ToolGuardRuntime:
    ...
    def __init__(
        ...
        Args:
            ...
            domain_dir: Optional custom domain directory path (defaults to .cuga/toolguard/domain)
        """
        self.tool_provider = tool_provider
        self.enable_policies = enable_policies
        self.policy_storage = policy_storage
        self.domain_dir = domain_dir or (Path.cwd() / ".cuga" / "toolguard" / "domain")
        ...
        logger.debug(f"Created ToolGuardRuntime instance (enable_policies={enable_policies}, domain_dir={self.domain_dir})")
    ...
    def _load_runtime_domain(self, app_name: str) -> RuntimeDomain:
        """
        Load RuntimeDomain files saved by ToolGuardBuildtimeManager for a specific app.

        Reads domain files from the same location where buildtime saves them:
        .cuga/toolguard/domain/<app_name>/
    ...
    def _validate_domain_directory(self, domain_dir: Path) -> None:
        ...
        if not domain_dir.exists():
            raise RuntimeError(
                ...
                "Generate tool guard code first so ToolGuardBuildtimeManager saves the domain files."
            )
