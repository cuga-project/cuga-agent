...
async def resolve_model_with_bind_tools(
    ...
    *,
    ...
    - ``cuga_lite_bind_tools_mode``: ``none`` | ``find_tools`` | ``all`` | ``apps`` | ``tools``
    - ``cuga_lite_bind_tools_apps``: list of app names (``mode=apps``)
    - ``cuga_lite_bind_tools_tool_names``: StructuredTool ``name`` values (``mode=tools``, e.g. ``load_skill``)
    - ``cuga_lite_bind_tools_include_find_tools``: merge ``find_tools`` into ``all`` / ``apps`` / ``tools``
    ...
    try:
        ...
        if mode == "tools":
            ...
            try:
                ...
                logger.warning("bind_tools tools: get_all_tools failed: %s", e)
                return active_model
            ...
            if duplicates:
                logger.debug(
                    "bind_tools tools: duplicate tool names from provider (using first): %s",
                    sorted(duplicates),
                )
            bound = []
            seen: Set[str] = set()
        ...
        logger.warning("Unknown cuga_lite_bind_tools_mode: %s (use none|find_tools|all|apps|tools)", mode)
    except Exception as e:
...
async def create_update_todos_tool(
    ...
    async def create_update_todos_func(input_data: Any) -> TodosOutput:
