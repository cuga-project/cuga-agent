...
def create_skill_tools(registry: SkillRegistry) -> list[StructuredTool]:
    async def load_skill_impl(name: str) -> str:
...
def format_available_skills_block(registry: SkillRegistry) -> str:
    ...
    lines.append("")
    lines.append(
        "When a task matches a skill, call `await load_skill(\"name\")`, print its output, "
        "then follow the instructions — writing normal Python code as you would for any other task."
    )
    return "\n".join(lines)
