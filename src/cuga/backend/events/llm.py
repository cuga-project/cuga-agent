...
The concierge wants a strong tool-caller (events/docs/ARCHITECTURE.md); workers can be cheaper per-agent.
"""
...
class RITSChatModel(_BaseChatModel):
    ...
    def _ping(self) -> "RITSChatModel":
        try:
            httpx.get(
                f"{self.base_url}/{self.model_name}/ping",
    ...
    async def _agenerate(self, messages: List[BaseMessage], stop=None, **kwargs: Any) -> ChatResult:
        ...
        async with httpx.AsyncClient() as client:
            resp = await client.post(url, headers={"RITS_API_KEY": self.api_key}, json=payload, timeout=180.0)
        ...
        return ChatResult(
            generations=[
                ChatGeneration(message=AIMessage(content=md.get("content") or "", additional_kwargs=extra))
            ]
        )
