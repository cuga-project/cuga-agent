...
EXAMPLES = [
    ...
    # ── newer trigger pieces — Google Calendar · Pinterest · YouTube · Discord reaction ───────────
    _ex("push-calendar-ends", "Meeting-recap watcher", "push",
        "when a meeting ends on my calendar, email me the follow-ups",
        agent="mailbot", channel="email", integration="google_calendar", phase="sprint", live=False,
        ap_trigger="event_ends",
        note="Google Calendar event_ends → mailbot → Gmail (connect in the AP Connections UI)"),
    _ex("push-calendar-new", "New-event briefer", "push",
        "when a new event is added to my google calendar, brief me",
        agent="mailbot", channel="slack", integration="google_calendar", phase="sprint", live=False,
        ap_trigger="new_event", note="Google Calendar new_event → mailbot"),
    _ex("push-pinterest-pin", "Pinterest pin watcher", "push",
        "when there's a new pin on my pinterest board, share it in Slack",
        agent="feed_watcher", channel="slack", integration="pinterest", phase="sprint", live=False,
        ap_trigger="newPinOnBoard", note="Pinterest new_pin (needs a board id) → feed_watcher"),
    _ex("push-youtube-video", "YouTube upload watcher", "push",
        "when @Fireship posts a new video, summarize it for me",
        agent="youtube_research", channel="slack", integration="youtube", phase="sprint", live=False,
        ap_trigger="new-video",
        note="YouTube new_video — public channel feed, NO OAuth (just the channel handle)"),
    _ex("push-rss-item", "RSS feed watcher", "push",
        "when a new item appears in https://blog.example.com/rss, summarize it in slack",
        agent="feed_watcher", channel="slack", integration="rss", phase="sprint", live=False,
        ap_trigger="new-item",
        note="RSS new_item — any public feed URL, NO OAuth; testable via /api/events/synth-fire"),
    _ex("push-discord-react", "Discord reaction watcher", "push",
        "when someone reacts with :bug: in #incidents on discord, triage the message",
    ...
    _ex("adv-gh-star-watch", "GitHub · repo lifecycle (star)", "push",
        "when my repo gets a new star, post a thank-you note with the stargazer's name",
        agent="repo_watcher", channel="slack", integration="github", phase="run", live=False,
        ap_trigger="trigger_star",
        note="repo_watcher announces lifecycle events (stars/releases/branches/milestones…)"),
    _ex("adv-gh-branch-watch", "GitHub · repo lifecycle (branch)", "push",
        "when a new branch is created on my repo, announce it and check the naming convention",
        agent="repo_watcher", channel="slack", integration="github", phase="run", live=False,
        ap_trigger="new_branch",
        note="repo_watcher · new_branch → announce + convention check"),
