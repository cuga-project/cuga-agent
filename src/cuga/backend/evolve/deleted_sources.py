...
async def deliver_source_deletions():
    ...
    if not events:
        return
    status = await EvolveIntegration.get_compliance_status(namespace_id=events[0]["instance_id"])
    if not supports_durable_retention(status or {}):
        # Preserve receipts for later delivery if the service moves to PostgreSQL.
        return
    for event in events:
