...
Some tests here (e.g. TestBuildAgentsFromStoredSubAgents) call reset_config_db() / save_config(),
which write to DBS_DIR/cuga.db — the SAME on-disk file a locally running `cuga start manager`
server uses (src/cuga/backend/storage/facade.py's _local_db_path()). Running these tests while a
dev server is up deletes/corrupts that shared file — this happened twice during the #101
supervisor work. Redirect every test in this directory to a throwaway temp directory instead.
"""
